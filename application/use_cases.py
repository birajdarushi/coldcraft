...
class CreateDraftUseCase:
    ...
    def _preflight_checks(self, request: CampaignRequest) -> None:
        ...
        if today_count >= policies.DAILY_SEND_LIMIT:
            raise DailyLimitError(
                f"Daily send limit ({policies.DAILY_SEND_LIMIT}) reached. Emails will be deferred to next available slot."
            )

        company = request.company_intel.get("company_name", "")
        company_count = self.campaigns.sent_to_company_30d(company)
        if company_count >= policies.MAX_COMPANY_EMAILS_30D:
            raise CompanyLimitError(
                f"Already sent {company_count} emails to {company} in the last 30 days. Max is {policies.MAX_COMPANY_EMAILS_30D}."
            )
        ...
        if match_score is not None and match_score < policies.MIN_MATCH_SCORE:
            logger.warning(
    ...
    def _self_review(self, draft: DraftResult) -> None:
        ...
        if len(words) < policies.MIN_WORDS:
            failures.append(f"Word count {len(words)} is below minimum {policies.MIN_WORDS}")
        if len(words) > policies.MAX_WORDS:
            failures.append(f"Word count {len(words)} exceeds maximum {policies.MAX_WORDS}")

        if len(draft.subject) > policies.MAX_SUBJECT_CHARS:
            failures.append(f"Subject '{draft.subject}' is {len(draft.subject)} chars (max {policies.MAX_SUBJECT_CHARS})")
        ...
        if len(draft.personalization_signals) < policies.MIN_PERSONALIZATION:
            failures.append(
                f"Only {len(draft.personalization_signals)} personalization signals (minimum {policies.MIN_PERSONALIZATION})"
            )
        ...
        if exclamation_count > policies.MAX_EXCLAMATIONS:
            failures.append(f"Too many exclamation marks ({exclamation_count}) — max {policies.MAX_EXCLAMATIONS}")
    ...
    def _qa_gate(self, draft: DraftResult, request: CampaignRequest) -> dict:
        ...
        for attempt in range(policies.QA_MAX_RETRIES + 1):
            ...
            if attempt < policies.QA_MAX_RETRIES:
                ...
                raise QAEscalationError(
                    f"QA gate failed {policies.QA_MAX_RETRIES + 1} times. Final violations: {result['violations']}. Escalating to PM Agent."
                )
